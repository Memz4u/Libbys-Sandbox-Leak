-- "lua\\weapons\\weapon_lenn_slowspinbotter.lua"
...
SWEP.Primary.Delay            = 100000000000000000000000000000000000000
...
function SWEP:Think()
	...
	function CalcThirdperson(ply, pos, ang, fov)
     if self.On == true then
			...
			view.origin = pos - Vector(-100,0,-60);
			view.angles = Angle(30,180,0);
			view.fov = fov;
...
timer.Create( "aimbotspin", 0.1, 0, function() 

  local ent = self.Owner
 local eyeang = ent:EyeAngles()

   eyeang.pitch =  math.Rand(-180,180)
   
   
    eyeang.yaw =  math.Rand(-360,360)
