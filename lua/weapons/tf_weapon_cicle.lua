-- "lua\\weapons\\tf_weapon_cicle.lua"
...
SWEP.PrintName = "Cicle (don't use for trickstab)"
...
SWEP.Primary.Sound = Sound( "weapons/icicle_hit_world_01.wav" )
...
dmg:SetDamage( tr.Entity:Health() * 0.99 )


timer.Simple(0.01, function() if( CurTime() < self.NextStrike ) then return; end
 
        self.NextStrike = ( CurTime() + 0.6 );
        
        if( CLIENT ) then return; end
 
        local trace = self.Owner:GetEyeTrace();
 
        local dis = self:GetPos():Distance( self:GetPos() )

        if( not trace.Entity:IsValid() ) then
              if dis <= 1000000000000000000000000000 then
               self.Owner:ChatPrint("You weren't looking at him, Therefore he didn't turn into ice")
               else
               self.Owner:ChatPrint("[ERROR]: WTF?")
               return;
        end
end

        if( self.Owner:EyePos():Distance( trace.Entity:GetPos() ) > 100 ) then
                self.Owner:ChatPrint("You seemed to be too far. Therefore he didn't turn into ice")
                return;
        end
        ...
        if( SERVER ) then 
                ...
                if( hp >= 100 ) then self.Owner:ConCommand("ulx asay (This is a automated message sent to the admin chat that I backstabbed someone with a cicle and he did not lose hp.)") self.Owner:ConCommand("say [AUTOMATED MESSAGE] Uh oh! I've been caught!") self.Owner:ChatPrint("[LIBBYSANTICHEAT] The enemy did not receive enough damage to be at a very low hp, Therefore he was not turned into ice. So tell me, are you trying to cheat the system? If this is a error. Please tell staff about it.")
 return; end
                if( hp <= 100 ) then self:MakeIce( tr.Entity ) end
...
timer.Simple( 0.8, function() if not IsValid(self) or not self.Owner:Alive() then return end local vm = self.Owner:GetViewModel()
	vm:ResetSequence( vm:LookupSequence( "eternal_idle" ) ) end)
