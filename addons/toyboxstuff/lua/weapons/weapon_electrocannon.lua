-- "addons\\toyboxstuff\\lua\\weapons\\weapon_electrocannon.lua"
...
function SWEP:PrimaryAttack()
	...
	local fbullet = {}
	fbullet.Src	 = self.Owner:GetShootPos()
	fbullet.Attacker = self.Owner
	fbullet.Dir	 = self.Owner:GetAimVector()
	...
	self.Owner:ViewPunch(Angle(math.Rand(-1.5,-2.5), math.Rand(-1,1), 0))
...
function EFFECT:Render() return true end
...
function EFFECT:Init( data )
	...
	local emitter=ParticleEmitter( Offset )
		for i=1,25 do
			bolt=emitter:Add( CreateMaterial("electrocannon_bluelight", "UnlitGeneric", { ["$basetexture"] = "sprites/bluelight1", ["$additive"] = 1 }), Offset )
			if(bolt)then
...
function EFFECT:Render( )
	...
	for i=1, 2 do
		render.DrawBeam( self.StartPos,
				 self.EndPos,
				 Lerp((self.EndTime-CurTime())*5,0,15),
				 texcoord,
...
function EFFECT:Render( )
	...
	for i=1, 3 do
		render.DrawBeam( self.StartPos,
				 self.EndPos,
				 Lerp((self.EndTime-CurTime())*3.33,0,20),
				 texcoord,
				 texcoord + self.Length / 128,
				 Color( 255, 100, 0, 255 ) )
	end
