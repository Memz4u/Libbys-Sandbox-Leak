-- "addons\\toyboxstuff\\lua\\weapons\\weapon_freezeray.lua"
...
---------------------------------------------------------*/
function SWEP:Think()    
    if self.Owner:KeyDown(IN_ATTACK) and !self.dt.imploding then
        self.dt.firing=true 
    else
        self.dt.firing=false
    end
    if self.dt.firing and self.Owner:KeyDown(IN_ATTACK) and !self.dt.imploding then
        local pos = self.Owner:GetShootPos()
        local ang = self.Owner:GetAimVector()
        local tdat = {}
        tdat.start = pos
        tdat.endpos = pos+(ang*1024)
        tdat.filter = self.Owner
...
function SWEP:SecondaryAttack()
    if !self.dt.imploding then
        ...
        self.Sound2:Play()
        timer.Simple(2, function() if IsValid(self) then self:Implode() end end)
...
function SWEP:Implode()
    if IsValid(self) and IsValid(self.Owner) and SERVER then
        for k,v in pairs(ents.FindInSphere(self:GetPos(),512)) do
            ...
            local ED = EffectData()
            ED:SetStart(self.Owner:GetPos()+self.Owner:OBBCenter())
            util.Effect("zeroxone_areaboom",ED)
            self.Sound2:Stop()
            self.dt.imploding = false
        end
        self.Owner:EmitSound("weapons/physcannon/energy_disintegrate4.wav")
    end
end
...
if CLIENT then
    ...
    hook.Add("RenderScreenspaceEffects","FreezeOverlay",function()
            ...
            if CurTime()>(GLOBALFREEZEENTSNEXTTHINK or 0) then
                ...
                for k,v in pairs(GLOBALFREEZEENTS) do
                    if IsValid(v) then
                        v:DrawModel()
                    end
                end
...
function SWEP:Initialize()
    ...
    if CLIENT then
        ...
        function EFFECT:Render()
            ...
            if !(self.ent) or !self.ent:GetOwner():GetActiveWeapon()==self.ent then
                return false
            end
        end
        ...
        function EFFECT3:Init(data)
            ...
            self.em = ParticleEmitter(pos)
            for i=1,10 do
                local tab = {"particle/smokesprites_0016",
