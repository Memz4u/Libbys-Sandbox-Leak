-- "addons\\toyboxstuff\\lua\\weapons\\weapon_hominglauncher.lua"
...
if self:GetNWEntity("target"):IsValid() then
    ...
    if (self:GetPos()-ent:GetPos()):Length()/16000 < 0.012 then size = ent:BoundingRadius()*2+50 end
    local offset = size/2
...
if tr.HitNonWorld then
    ...
    if tr.Fraction < 0.012 then size = tr.Entity:BoundingRadius()*2+50 end
    local offset = size/2
...
local ents = ents.FindByClass("sent_hmissile")
local entscr
local entvel
...
if !entscr.visible then break end
...
--    pos = pos + vec
    ...
    --pos = pos:Rotate(self.Owner:EyeAngles())
...
if !self:GetNWEntity("target"):IsValid() or !self:CanPrimaryAttack() then
    self:EmitSound( FailSound )
return end
    
    local effectdata = EffectData()
        effectdata:SetOrigin( self.Owner:GetShootPos()+self.Owner:GetAimVector()*65 )
    ...
    local ent = ents.Create( "sent_hmissile" )
        ...
        ent:SetPos( self.Owner:GetShootPos( ) + self.Owner:GetAimVector() * 55 )
        ent:SetAngles(self.Owner:GetAimVector():Angle())
        ent:SetVelocity(self.Owner:GetAimVector()*750)
   ...
   -- ent:Fire("kill",1,9)
    ...
    self.Owner:ViewPunch( Angle( -6, 0, 4 ) ) 
end
