-- "addons\\toyboxstuff\\lua\\weapons\\weapon_tmrcolt\\shared.lua"
...
function SWEP:Reload()
	...
	if self:Clip1() == 6 and self:Clip2() == 6 then
		return false
	end
...
function SWEP:Holster()
	...
	local viewmodel2 = self.Owner:GetViewModel(0)

	if (IsValid(viewmodel1)) then
		viewmodel1:SetWeaponModel(self.ViewModel, nil)
	end

	if CLIENT then -- only tested with bots, might not work on real players?¨
		if (IsValid(viewmodel2)) then
...
function SWEP:ViewModelDrawn(vm)
	local viewmodel_left = self.Owner:GetViewModel(1)
...
function SWEP:ImpactEffect(tr, dmgtype)
	...
	decoy.Num = 1
	decoy.Src = self.Owner:GetShootPos()
	decoy.Dir = self.Owner:GetAimVector()
...
function SWEP:SendViewModelAnim( act , index , rate ) --taken from gmod wiki, don't really know howto do dual weapons, sorry :(
