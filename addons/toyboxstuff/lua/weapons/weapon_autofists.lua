-- "addons\\toyboxstuff\\lua\\weapons\\weapon_autofists.lua"
...
function SWEP:PrimaryAttack()
    ...
    if tr.HitPos:Distance(self.Owner:GetShootPos()) < 70 then
        ...
        if self.Special == 1 then
            self:FireBullets({
                    ...
                    Damage = 1000,
                    Attacker = self.Owner,
                    Callback = function() self:EmitSound(tostring(table.Random(self.SwingHit))) end
            });
        else
            self:FireBullets({
                    Num = 1,
