-- "addons\\toyboxstuff\\lua\\weapons\\weapon_pixelsword.lua"
...
umsg.Start( "pixel_weapon_menu", self:GetOwner() )
umsg.Char( self.Damage - 128 )
umsg.Float( self.SwingDelay )
umsg.End()
...
usermessage.Hook( "pixel_weapon_swing", Swing )
...
local dmg = msg:ReadChar() + 128
local swingDelay = msg:ReadFloat()
...
if not IsValid( wep ) or not wep.PixelWeapon then menu:SetVisible( false ) return end
...
"Right-Click to remove the specific part.",
"Hold down C while left clicking to copy a color instead of replacing it.",
"The alpha in the color mixer is not working."
...
usermessage.Hook( "pixel_weapon_menu", Menu )
