-- "addons\\toyboxstuff\\lua\\weapons\\weapon_rocketbow.lua"
...
SWEP.VElements = {
	["rocket"] = { type = "Model", model = "models/Weapons/W_missile_closed.mdl", bone = "ValveBiped.bolt", rel = "", pos = Vector(0, 0, 10), angle = Angle(90, 0, 0), size = Vector(1, 0.5, 0.5), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
...
function SWEP:Think()
	if(self.NeedsReload)then self:Reload() end
end
...
function SWEP:ShootRocket()
	...
	rocket:GetPhysicsObject():AddVelocity(self.Owner:GetAimVector()*2000)
	end
	rocket.Inflictor=self
	rocket.Attacker=self.Owner
...
function SWEP:PrimaryAttack()
	...
	self.Owner:ViewPunch(Angle(math.Rand(4,5),math.Rand(-1,1),0))
	self:EmitSound(self.Primary.Sound)
	self.Owner:SetAnimation(PLAYER_ATTACK1)
	if(self:Clip1()==0)then self.NeedsReload=true end
end
...
usermessage.Hook("ExplodeDynamicLight",function(um)
	local dlight = DynamicLight(um:ReadFloat())
	if ( dlight ) then
		dlight.Pos = um:ReadVector()
		dlight.r = 255
		dlight.g = 150
		dlight.b = 0
		dlight.Brightness = 1
