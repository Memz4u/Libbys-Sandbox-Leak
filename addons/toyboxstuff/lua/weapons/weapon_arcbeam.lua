-- "addons\\toyboxstuff\\lua\\weapons\\weapon_arcbeam.lua"
...
list.Add("NPCUsableWeapons", {class = "weapon_arcbeam", title = "Arc Beam"})
...
local function soldierfix(ent,timerid)
    if IsValid(ent) && ent:GetActiveWeapon():IsValid() then
        //ent:SetSchedule(96)
...
function SWEP:DoSoldierFix()
    ...
    if IsValid(self.Owner) and self.Owner:GetClass() == "npc_combine_s" then
        self.SetupWeaponHoldTypeForAI = SoldierAnims
        self:SetupWeaponHoldTypeForAI()
        //self:SetWeaponHoldType("ar2")
        local id = "arc_csolfix_"..self.Owner:EntIndex()
        timer.Create(id, 2, 0, function() if IsValid(self) then soldierfix(self.Owner, id) end end)
    end
...
function SWEP:Think()
    ...
    if self.Owner:KeyDown(IN_ATTACK) && (self.HeatTime != 0) then
    ...
    if self.MouseDown && IsFirstTimePredicted() && !self.Owner:KeyDown(IN_ATTACK) then
        self:TurnOff()
    end
    if SERVER && self:GetHeat() == 10 then
        local edata = EffectData()
        edata:SetOrigin(self.Owner:GetShootPos())
        util.Effect("Explosion",edata,nil,true)
        util.BlastDamage(self,self.Owner,self.Owner:GetShootPos(),200,130)
        self:Remove()
    end
    self.LastThink = CurTime()
end
...
local function checkifshouldstayon(wep)
    if IsValid(wep) then
        if !IsValid(wep:GetOwner()) || !wep:GetOwner():IsNPC() || (wep.LastNPCShoot+1 < CurTime()) then
            wep:TurnOff()
    elseif IsValid(wep.Beam) && (wep.Beam.dt.endent != NULL) && (wep.Beam.dt.endent != wep:GetOwner():GetEnemy()) then
            local en = wep:GetOwner():GetEnemy()
            if !IsValid(ent) || wep:GetPos():Distance(en:GetPos()+en:OBBCenter()) > wep.Range then
                wep:TurnOff()
            else
...
function SWEP:NPCShoot_Primary(ShootPos,ShootDir) --messing around with this
    if !IsValid(self.Owner) || !IsValid(self.Owner:GetEnemy()) then
        return
    end
    local en = self.Owner:GetEnemy()
    ...
    if dist > self.Range then
        ...
        return
    elseif dist < 80 then
        self.Owner:SetSchedule(SCHED_RUN_FROM_ENEMY_MOB)
        return
    end
...
function SWEP:TurnOff()
    if SERVER && IsValid(self.Beam) then
        self.Beam:Kill()
        self.Beam = NULL
    end
...
if CLIENT then
    ...
    function SWEP:DrawWorldModel()
        ...
        render.SetColorModulation(1+heat,1+heat/2,1+heat/2)
        local baseillum = 0.25
        if IsValid(self.Owner) && self.Owner:IsPlayer() && self.Owner:KeyDown(IN_ATTACK) then
            baseillum = 6.75
        end
        vec_illum.x = heat/2+baseillum
        vec_illum.y = heat/20+baseillum
    ...
    surface.CreateFont("HUDNumber5", {font = "Trebuchet MS", size = 45, weight = 800})
end
