-- "addons\\toyboxstuff\\lua\\weapons\\weapon_teslagun.lua"
...
for k,v in pairs(ents.FindInSphere(self.Owner:GetPos()+self.Owner:GetForward()*300,350)) do 
if (v:IsPlayer() or v:IsNPC()) and v != self.Owner and self.ChoseTarget == false and (v:Health() != 0 or v:GetClass() == "npc_rollermine") then
...
local tr = self.Owner:GetEyeTrace()
if self.Owner:GetPos():Distance(tr.HitPos) > 500 then
pos = self.Owner:GetPos()+self.Owner:GetAimVector()*500+Vector(0,0,60)
else
pos = tr.HitPos
end
if (SERVER) then
