-- "addons\\toyboxstuff\\lua\\weapons\\weapon_gunstrumental.lua"
...
-- and yet years later, this hurts so much for me to do mp support that i can't, sorry :(
SWEP.Cursor = 1
...
function SWEP:Initialize() 
	if CLIENT then
		chat.AddText(Color(255, 255, 0), "The Gunstrumental", Color(255, 255, 255), " is a weapon that shoots music. ", Color(0, 255, 0), "Right click to open the menu ", Color(255, 255, 255), " and go ahead, load some music sheets!" )
	end
...
function SWEP:PrimaryAttack()
	...
	local could = false
	if self:PrimaryPattern() then
		self:FireA()
		could = true
	end

	if self:HarmonicsPattern() then
		self:FireB()
		could = true
	end
...
function SWEP:Think()
	if CLIENT then
		...
		if self.LastCursorPos == -1 then
			timer.Simple(1.5, function(self) if IsValid(self) then GUNSTRU:RequirePatternUpdate( LocalPlayer() ) end end, self )
	...
	if SERVER then
		if self.NeedsInit then
			...
			timer.Simple(1.5, function(self) if IsValid(self) then GUNSTRU:RequirePatternUpdate( self.Owner ) end end, self )
...
function SWEP:PatternPitch( ent, iPattern )
	...
	print(pitch)
	if SERVER then
		self:SetNWString("sNote", sNote)
		self:SetNWFloat("pitch", GUNSTRU:GetTonePitch( sNote, iOffs ))
		self:SetNWFloat("iOffs", iOffs)
	end
...
function SWEP:PatternPlay( ent, iPattern, opt_bShouldAlert )
	...
	local pitch, snote = self:GetNWFloat("pitch"), self:PatternPitch( ent, iPattern )
	print(pitch)

	if isnumber(pitch) and self:GetNWString("sNote") ~= ".." then
		if pitch <= 255 and SERVER then
			print(pitch)
			self:EmitSound( GUNSTRU:RetreiveSound( ent, iPattern ), 50, self:GetNWFloat("pitch"), 1, CHAN_STATIC)
		elseif opt_bShouldAlert then
			--print( "Tried to play note ".. self:GetNWFloat("sNote") .." which is, with offsets ".. self:GetNWFloat("iOffs") .. " too high to be played (" .. math.floor(pitch) .. ") !")
			return false
		end
...
function GUNSTRU:BuildGunstrumentalTable( ply )
	...
	if not IsValid( ply ) or ply.GS then return false end
...
function GUNSTRU:GetHitRatio( ply, iNum )
	if not IsValid( ply ) or not ply.GS then return 0 end
...
function GUNSTRU:RetreivePattern( ply, iNum )
	if not IsValid( ply ) or not ply.GS then return "" end
...
function GUNSTRU:RetreiveTempo( ent )
	...
	return ent:GetInfoNum( "gs_tempo", 0)
...
function GUNSTRU:RetreiveGlobalOffsets( ent )
	...
	return ent:GetInfoNum( "gs_offsets", 0 )
...
function GUNSTRU:RetreiveOffset( ent, iNum )
	...
	return ent:GetInfoNum( "gs_".. self:UTIL_IntegerToAZCode( iNum ) .. "offset", 0 )
...
function GUNSTRU:RetreiveTrans( ent, iNum )
	...
	return (ent:GetInfoNum( "gs_".. self:UTIL_IntegerToAZCode( iNum ) .. "trans", 0) ) * 12
...
function GUNSTRU:Gunstrumentalize( tab, harmonics )
	...
	while iPattern < GUNSTRUMENTAL_NUM_VARS do
		iPattern = math.Clamp(iPattern + 1, 0, 100)
		print(iPattern)
		print("pattern")
		print(tab)
		if tabpos < tablen then
			RunConsoleCommand("gs_zzapat"..iPattern, string.sub( tab, tabpos, tabpos + size - 1 ) )
			chat.AddText( "setting "..iPattern)
			RunConsoleCommand("gs_zzbpat"..iPattern, string.sub( harmonics, tabpos, tabpos + size - 1 ) )
			tabpos = tabpos + size
		else
...
if CLIENT then
	...
	function GUNSTRU:BuildContents()
        ...
        do
            local item = "closebox"
            header.CTS[item] = vgui.Create("DButton", header)
            header.CTS[item]:SetText( "X" )
        ...
        header.PerformLayout = function( self )
            ...
            self.CTS["closebox"]:SetSize( self:GetTall() * 0.5, self:GetTall() * 0.9 )
		...
		do
			...
			do
				...
				form.CTS[item].Think = function( self )
					...
					if IsValid( ent ) and (ent ~= self.LastEnt) and ent.GetGunstrumental and (type(ent.GetGunstrumental) == "function") then
						self.LastEnt = ent
						self:Clear()
		...
		do
			...
			do
				...
				table.insert( sets,
					{ "Bonk",
						"vo/scout_specialcompleted03.mp3"	, 0,
						"vo/scout_specialcompleted02.mp3"	, -1
					} )
				table.insert( sets,
					{ "Music Box",
				...
				form.CTS[item] = vgui.Create( "DComboBox" )
		...
		do
			...
			do
				...
				form.CTS[item].Think = function( self )
					...
					local found = IsValid(self.GunstrumentalObject)
					if not found then
						...
						while not found and (iSearch <= #tWeapons) do
							if tWeapons[iSearch]:GetClass() == GUNSTRU_WeaponClassname then
								found = IsValid( self.GunstrumentalObject )
...
Please note that if your pattern is short, reloading your weapon may take less time.]] )
				form.CTS[item]:SetWrap( true )
				form.CTS[item]:SetAutoStretchVertical( true )
				form:AddItem( form.CTS[item] )
		...
		do
			...
			do
				...
				form.CTS[item].DoClick = function( self )
					local url = self.form.CTS["url"]:GetValue()
					local callback = function(contents)
						local hack = self.form.CTS["edit"]
						hack:SetValue( contents )
					end
		...
		tabMaster:AddSheet( "General", tabMaster._p_general, "icon16/application_view_detail.png", false, false, "General options." )
		tabMaster:AddSheet( "Instruments", tabMaster._p_instruments, "icon16/application_view_detail.png", false, false, "Tweak the way instruments sound like." )
		tabMaster:AddSheet( "Text downloader", tabMaster._p_downloader, "icon16/application_view_detail.png", false, false, "Get music from URLs here!" )
		tabMaster:AddSheet( "Maker", tabMaster._p_maker, "icon16/application_view_detail.png", false, false, "Make your own music here!" )
		tabMaster:AddSheet( "Trimmer", tabMaster._p_trimmer, "icon16/application_view_detail.png", false, false, "If you're experienced, use this tool to create mashups by trimming them!" )
		tabMaster:AddSheet( "Merger", tabMaster._p_merger, "icon16/application_view_detail.png", false, false, "Merge patterns together using this tool and try them!" )
		tabMaster:AddSheet( "Transposer", tabMaster._p_transposer, "icon16/application_view_detail.png", false, false, "You can transpose patterns here!" )
