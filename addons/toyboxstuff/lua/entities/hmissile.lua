-- "addons\\toyboxstuff\\lua\\entities\\hmissile.lua"
...
if self:GetNWInt("det")<CurTime() then self:Explode(self) end
local ent = self:GetNWEntity("target")
	if not ent:IsValid() then self:SetVelocity(Vector(0,0,-220)) return end
	if (ent:LocalToWorld(ent:OBBCenter())-self:GetPos()):Length()<70 then self:Explode(self) return end
