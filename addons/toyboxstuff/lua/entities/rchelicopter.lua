-- "addons\\toyboxstuff\\lua\\entities\\rchelicopter.lua"
...
function ENT:Use(ply, cal)
    ...
    if !self.InUse && self.Healthz>0 && AngTest(self.Entity:GetAngles()) && !self.IsLocked  then
        self.InUse = true
        self.User = ply
...
function ENT:OnRemove()
    ...
    if self.User then self.User:SetColor(Color(255,255,255,255)) end
    self.User = nil
    self.Sound:Stop()
    self.Entity:StopMotionController()
    self.Entity:SetNWBool("USE",false)
...
function ENT:OnTakeDamage(dmg)
    ...
    if self.User then self.User:SetNWInt("HHealthz", self.Healthz) end
end
