-- "addons\\toyboxstuff\\lua\\entities\\questv2.lua"
...
function ENT:SpawnFunction( ply, tr )
    ...
    ShopCrew:SetModel(table.Random({"models/Humans/Group02/Female_01.mdl","models/Humans/Group02/Female_02.mdl",
    "models/Humans/Group02/Female_03.mdl","models/Humans/Group02/Female_04.mdl","models/Humans/Group02/Female_06.mdl","models/Humans/Group02/Female_07.mdl","models/Humans/Group02/Male_01.mdl","models/Humans/Group02/Male_02.mdl","models/Humans/Group02/Male_03.mdl","models/Humans/Group02/Male_04.mdl","models/Humans/Group02/Male_05.mdl","models/Humans/Group02/Male_06.mdl","models/Humans/Group02/Male_07.mdl"}))
...
function Quest:Add(description,info,xp,min,max,ply)
    ...
    xp = tostring(xp)
    ply = ply or NULL
    if ply != NULL && ply:GetNWInt(info,0) >= min and ply:GetNWInt(info,0) <= max and !table.HasValue(Quest.Forbidden,ply:GetActiveWeapon():GetClass()) and ply:Alive() then
        ...
        for k,v in pairs(Quest.LvlXP) do
        	print(k)
        	print(v)
    ...
    if ply:Alive() and ply != NULL && ply:GetNWInt(info,0) == max and !table.HasValue(Quest.Forbidden,ply:GetActiveWeapon():GetClass()) then
...
hook.Add("EntityTakeDamage","Quest_Gnome", function( ent, dmginfo ) -- Requires Hds46's gnome at https://steamcommunity.com/sharedfiles/filedetails/?id=950713693
...
hook.Add("PlayerSay", "Achievement_Quest", function(ply,txt)
    ...
    if ((txt):sub(1,5) == "/shop" || (txt):sub(1,5) == "/Shop") then
    ply:ConCommand("Quest_Shop")
    return "[Looking at the Shop Menu]"
    end
    
    if ((txt):sub(1,11) == "/Quest_Book" || (txt):sub(1,11) == "/quest_book" || 
    (txt):sub(1,5) == "/Book" || (txt):sub(1,5) == "/book")  then
    ply:ConCommand("Quest_Book")
...
local Shop = {
    ...
    name = "Trail",
    price = 1000,
    func = function(ply,c,Col) Truncate = util.SpriteTrail(ply, 0, Col, false, 15, 1, 4, 1/(15+1)*0.5, "trails/plasma.vmt") end,
    },
}
...
concommand.Add("shop_buy",function(ply,cmd,args)
    ...
    for k,v in pairs(ShopWeapon) do
        if string.find(tostring(args[1]),v["name"]) and ply:GetNWInt("MoneyEarned",0) >= v["price"] then
        if args[2] == "Ak47" then
            choosewep = "tb_ak47"
        ...
        print(args[1],args[2])
        if string.find(args[2], "Ammo") && ply:GetNWInt("MoneyEarned",0) >= v["price"] + 300 && ply:GetWeapon("tb_ak47") || 
        ply:GetWeapon("tb_m4") ||
        ply:GetWeapon("tb_deagle") || 
        ply:GetWeapon("tb_fiveseven") || 
        ply:GetWeapon("tb_glock") || 
        ply:GetWeapon("tb_mac10") || 
        ply:GetWeapon("tb_mp5") ||
        ply:GetWeapon("tb_para") ||
        ply:GetWeapon("tb_pumpshotgun") ||
        ply:GetWeapon("tb_tmp") then
...
if CLIENT then
    ...
    usermessage.Hook("Quest_NPC",  function(um)
        description = um:ReadString()
        monsters = um:ReadShort()
    end)
