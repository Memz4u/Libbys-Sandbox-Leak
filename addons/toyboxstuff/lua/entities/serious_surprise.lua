-- "addons\\toyboxstuff\\lua\\entities\\serious_surprise.lua"
...
if (!_G.Ghor.CreateEntList) then //Dont run this if the system is already initialized!
	...
	local EntList = {};
		...
		*/
		...
		local function OnEntityRemoved(ent)
			...
			for _, v in pairs(boundLists) do
				v:RemoveEntity(ent);
			end
		...
		function EntList:AddEntity(ent)
			if (!table.HasValue(self.Items, ent)) then
				...
				if (!TableBindsByEntity[ent]) then
		...
		function EntList:RemoveEntity(ent)
			for i, entCompare in pairs(self.Items) do
		...
		function EntList:Remove()
			...
			for _, ent in pairs(self.Items) do
				for i, listCompare in pairs(TableBindsByEntity[ent]) do
...
if (SERVER) then
	...
	hook.Add("OnNPCKilled", "SeriousSurpriseNPCDead", function(ent, attacker, inflictor)
		if (ent.IsKamikaze && IsValid(ent.Spawner)) then
...
if (CLIENT) then
	//Nothing
elseif (!(Ghor.EntPropagator)) then
		...
		//===============================================
			...
			//-----------------------------------------------
				...
				function Wave:Begin()
					...
					if ((self.m_pSpawnType == WAVELOCATION_PERSISTENT) || (self.m_pSpawnType == WAVELOCATION_PERSISTENT_DELEGATEONLY)) then
						self:SelectSeed();
					end
				end
				...
				function Wave:Spawn()
					...
					local seed = self.m_pSpawnSeed || NULL;
					if (spawntype == WAVELOCATION_RANDOM) then
						seed = self:SelectSeed();
						if (!seed) then
							return NULL; //No seeds were ready to spawn.
						end
						if (!(seed:IsValid())) then
				...
				function Wave:Update()
					local ctime = CurTime();
					if ((!self.m_bActive) && (self.m_flWaveTime <= ctime)) then
						self:Begin();
					end
			...
			//-----------------------------------------------
				...
				function Wave:SetAvailableSeeds(seeds, spawntype)
					...
					if ((spawntype != WAVELOCATION_RANDOM) && (!table.HasValue(seeds, self.m_pSpawnSeed))) then
						self:SelectSeed();
					end
				end
	...
	function EntPropagator:CreateSeed(pos)
		if (self.m_iSeedCap == 0) then
			return;
		end
		if (#self.m_t_pSeeds >= self.m_iSeedCap) then //Bump out the least favorable if we are at the limit.
			local lowestFavorability = self.m_t_pSeeds[1].m_flFavorability;
			local leastFavorable = self.m_t_pSeeds[1];
			for k,v in pairs(self.m_t_pSeeds) do
	...
	Ghor.EntPropagator.Seed = Seed;
		...
		function Seed:ExitRange(otherSeed)
			if (otherSeed:IsValid() && (!otherSeed.m_t_flSeedDistances[self])) then
				return;
			end
		...
		function Seed:UpdateRelationship(otherSeed)
			...
			if (otherSeedDistances[self]) then //Only update the other seed if were in its list of nearby seeds.
		...
		function Seed:SetRange(flRange)
			...
			if (flRange < self.m_flRange) then //our range has shrunk, only notify seeds that were within our range
				for otherSeed, distance in pairs(self.m_t_flSeedDistances) do
		...
		function Seed:AddSeeds()
			...
			for k, otherSeed in pairs(self.m_t_Container) do
				if ((otherSeed != self) && (!seedDistances[otherSeed])) then
					local distance = self:GetDistanceInRange(otherSeed);
					if (distance) then
		...
		function Seed:AddToSeeds()
			for k, otherSeed in pairs(self.m_t_Container) do
				if ((otherSeed != self) && (!otherSeed.m_t_flSeedDistances[self])) then
		...
		function Seed:GetDistanceInRange(otherSeed)
			...
			if ((math_abs(pos.x - otherpos.x) <= range) && (math_abs(pos.y - otherpos.y) <= range) && (math_abs(pos.z - otherpos.z) <= range)) then
		...
		function Seed:CanSpawn()
			...
			//Don't spawn if we've already spawned too recently.
				if (self.m_flBeginSpawnable > ctime) then
					return false;
				end
				
			//Test if the spawn area is clear
				if (!self:TestHullUnobstructed()) then
					//The hull is obstructed, we cant spawn here right now.
					return false;
				end
				
			//Test if the spawn area cant be seen.
				if (SinglePlayer()) then
					if (self:TestVis(Entity(1))) then
						//The player can see us, we cant spawn here right now.
						return false;
					end
				else
					for _,pl in pairs(player.GetAll()) do
						if (self:TestVis(pl)) then
							return false; //A player can see us, we cant spawn here right now.
						end
					end
				end
		...
		function Seed:PerformSpawn()
			...
			if (range != 0) then
				...
				for seed, distance in pairs(self.m_t_flSeedDistances) do
					if (distance != 0) && (distance <= self.m_flPenaltyRange) then
		...
		function Seed:WriteWorldBBoxCorner(dest, corner)
			...
			dest.x = pos.x + (((corner == 0) && mins.x) or maxs.x);
			dest.y = pos.y + (((corner == 0) && mins.y) or maxs.y);
			dest.z = pos.z + (((corner == 0) && mins.z) or maxs.z);
		end
		...
		local function DropHullToGround(pos, maxdistance, mins, maxs)
			...
			if ((tr.HitSky) || (tr.HitNoDraw) || (tr.MatType == MAT_SLOSH)) then //spawning on water/sky/clip is not allowed.
				return nil;
			end
			if (tr.Hit) then
