-- "addons\\toyboxstuff\\lua\\entities\\ghor_shuriken.lua"
...
hook.Add("CreateEntityRagdoll","TransferShurikens",function(ent,rag)
		for k,v in pairs(ents.FindByClass("ghor_shuriken")) do
...
if !brush && (tr.Entity != data.HitEntity) then
...
function ENT:ImpactEffect(tr)
	if !IsValid(self) or dodebug then
		return
	end
	local ef = EffectData()
