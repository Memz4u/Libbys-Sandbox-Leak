-- "addons\\toyboxstuff\\lua\\entities\\serioussurprise.lua"
...
function ENT:SpawnKamikaze()
	if !SERVER or self:GetNWFloat("spawntimer") > CurTime() or self:GetNWFloat("wave") ~= 1 then return end
	local navmesh = table.Random(navmesh.GetAllNavAreas())
	if navmesh:IsUnderwater() then self:SpawnKamikaze() return end
...
hook.Add("PlayerDeath", "Kamikaze_PlayerDeath", function()
	...
	if dead == table.Count(player.GetAll()) then
