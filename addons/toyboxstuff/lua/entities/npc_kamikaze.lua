...
function ENT:StartScreaming()
	...
	if !IsValid(self:GetNWEntity("enemy")) or self:GetNWBool("injured") then self:StopScreaming() return end
...
function ENT:AlertAllies(enemy)		
	...
	for k,v in pairs(items) do
		if (v:GetPos():Distance(pos) < 600) and IsValid(self:GetNWEntity("enemy")) and v ~= self then
			if !IsValid(v:GetNWEntity("enemy")) then
...
--[[	local legs = ents.Create("prop_ragdoll")
	...
	for k, v in pairs(ents.FindByClass("serioussurprise")) do
		if IsValid(v) then
...
function ENT:Move()
	if !IsValid(self:GetNWEntity("enemy")) then return end
...
if (CLIENT) then
	...
	function EFFECT:Think()
		local phys = self:GetPhysicsObject()
		if (phys:IsValid() && !phys:IsAsleep()) then
		...
		if !isnumber(self.nextSteam) then self.nextSteam = 15-0.1 end
		if !IsValid(self.em) then self.em = ParticleEmitter(self:GetPos()) end
