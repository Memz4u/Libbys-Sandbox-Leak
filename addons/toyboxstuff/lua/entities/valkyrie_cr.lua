-- "addons\\toyboxstuff\\lua\\entities\\valkyrie_cr.lua"
...
function Missile:PhysicsUpdate()
    ...
    if IsValid(self.Owner) then
        self:GetPhysicsObject():SetVelocity((self.Entity:GetForward()*1024)*self.SpeedMultiplier)
        ...
        self.Owner:SetNWVector("toybox_629_dtt",DTTTraceRes.HitPos)
        self.Owner:SetNWVector("toybox_629_rocketpos",self.Entity:GetPos())
...
function Missile:ExplodeMissile()
    ...
    if self:WaterLevel() >=1 then
        util.BlastDamage(self,self.Owner,self:GetPos(),256,256)
        util.Effect( "WaterSurfaceExplosion", MissileExpEffectData, true, true )
    else
        util.BlastDamage(self,self.Owner,self:GetPos(),512,512)
...
function Missile:OnRemove()
    ...
    if IsValid(self.Owner) then
        SendUserMessage("toybox_629_disablehooks",self.Owner)
    ...
    if timer.Exists("toybox_629_nofuel") then timer.Destroy("toybox_629_nofuel") end
end

function Missile:PhysicsCollide()
    if self.Exploded then return end
    self:ExplodeMissile()
end
...
function Missile:Boost()
    if SERVER then
        ...
        if self.Boosted then
            ...
            self.Boosted = false
            umsg.Start("toybox_629_fuelbarspeed",self.Owner)
                umsg.Bool(false)
            umsg.End()
...
function Missile:Initialize()
    if CLIENT then killicon.AddFont("toybox_629_valkyrie_cr","HL2MPTypeDeath","3",Color(255,80,0,255)) end
...
if CLIENT then
    ...
    local function ValkyrieHUDCreate()
        if timer.Exists("toybox_629_killhooks") then
            ...
            timer.Destroy("toybox_629_killhooks")
        end
        HUDMultiplier = 1
        ...
        SmallGap = false
        timer.Create("toybox_629_squaresize",0.5,0,function() SmallGap = !SmallGap end)
    ...
    local function ValkyrieHUDDestroy()
        ...
        if timer.Exists("toybox_629_squaresize") then timer.Destroy("toybox_629_squaresize") end
    ...
    usermessage.Hook("toybox_629_enablehooks",ValkyrieHUDCreate)
    usermessage.Hook("toybox_629_disablehooks",ValkyrieHUDDestroy)
    usermessage.Hook("toybox_629_fuelbarspeed",ValkyrieHUDUpdateFuelBarSpeed)
end
