-- "addons\\toyboxstuff\\lua\\entities\\ravebreak.lua"
...
if SERVER then
    ...
    function RaveEnd(stopsounds)
        if !Raving then return end
        hook.Remove("Think","RaveThink")
        umsg.Start("RaveEnd")
        umsg.Bool(stopsounds)
        umsg.End()
        Raving = false 
    end
...
if CLIENT then
    ...
    function RaveBreak(um)
        ...
        timer.Simple(0.1, function() surface.PlaySound(RAVESOUND) end)
        timer.Simple(1,function()
    ...
    function RaveEnd(um)
        if (um:ReadBool()) then RunConsoleCommand("stopsounds") end
