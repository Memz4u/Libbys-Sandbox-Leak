-- "addons\\toyboxstuff\\lua\\entities\\goodbye\\cl_init.lua"
...
usermessage.Hook( "GoodbyePack", function( um )
	local ent = um:ReadEntity()
	if !IsValid( ent ) then return end
	ent.Packed = um:ReadChar()
end )

usermessage.Hook( "GoodbyeDie", function( um )
	local ent = um:ReadEntity()
	if !IsValid( ent ) then return end
	...
	local function DramaticMsg( msg )
		if !LocalPlayer().Messages then return end
		local message = { Message = msg, Time = CurTime() }
		table.insert( LocalPlayer().Messages, message )
	end
	DramaticMsg( "After so many bouncy entities..." )
	timer.Simple( 3, function() DramaticMsg( "We're compelled to leave ToyBox." ) end )
	timer.Simple( 6, function() DramaticMsg( "Waiting, we'll be there..." ) end )
	timer.Simple( 9, function() DramaticMsg( "For when the ToyBox goes multiplayer." ) end )
	timer.Simple( 12, function() DramaticMsg( "That will be the day..." ) end )
	timer.Simple( 15, function() DramaticMsg( "Until then.  This is our goodbye. :(" ) end )
end )

usermessage.Hook( "GoodbyeDieRemove", function( um )
...
hook.Add( "CalcView", "DramaticCalcView", function( ply, pos, ang, fov )
	if IsValid( ply.DramaticEnt ) then
		...
		local tr = util.TraceLine( { start = center, endpos = offset, filter = ply.DramaticEnt } )
	...
	if LocalPlayer().Dramatic then
		...
		if LocalPlayer().Messages then
			for k,v in ipairs( LocalPlayer().Messages ) do
				...
				if timer > 3 then timer = 3 end
				if v.Time + 3 > CurTime() then timer = v.Time + 3 - CurTime() end
				...
				if v.Time + 3 < CurTime() then table.remove( LocalPlayer().Messages, k ) end
				end
			end
		end
end )
