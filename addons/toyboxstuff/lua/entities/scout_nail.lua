-- "addons\\toyboxstuff\\lua\\entities\\scout_nail.lua"
	...
	function NAIL:PhysicsCollide(data, physobj)
		...
		if data.HitEntity == self.OWNER then return end
			...
			if data.HitEntity:GetClass() != "worldspawn" then 
				...
				timer.Simple(0, function() if !IsValid(self) then return end self:SetMoveType( MOVETYPE_NOCLIP ) self:SetPos( self:GetPos() - self:GetAngles():Forward()*10 ) end)
				
			end
			
			if data.HitEntity:IsNPC() or data.HitEntity:IsPlayer() then
				
				timer.Simple(.2, function() if IsValid(self) and data and data.HitEntity:Health() <= 0 then self:Remove() end end)
