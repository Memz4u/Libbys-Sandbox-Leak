-- "addons\\toyboxstuff\\lua\\entities\\metropolice_mask\\cl_init.lua"
...
language.Add( "Undone_metropolice_mask", "Undone Metropolice Mask" )
	...
	copradio.Disable = function(self)self.State = RADIO_OFF end
	copradio.Add = function(self,sound)	table.insert(self.Queue,sound) end
	copradio.Pause = function(self,n) n = n or 1 for i = 1,n do table.insert(self.Queue,"_comma") end end
	...
	usermessage.Hook("MPMask", function()
		copradio:Enable(3.4)
	...
	hook.Add("Think", "MPMask", function()
		if !(copradio.State > RADIO_ONHOLD) then return end
		if !(CurTime() > copradio.NextThink) then return end
		local path
