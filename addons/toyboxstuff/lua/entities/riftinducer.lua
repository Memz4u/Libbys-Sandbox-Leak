-- "addons\\toyboxstuff\\lua\\entities\\riftinducer.lua"
...
if SERVER then
	...
	function ENT:Think()
		...
		if self.RemoveDelay < CurTime() && !self.IsDead then
		...
		for _, ent in ipairs( ents.GetAll() ) do
			...
			if IsValid( ent ) && ent != self:GetOwner() && ent != self:GetOwner():GetActiveWeapon() && !ent.IsRiftInducer then
				...
				if dist < 512 then
					...
					if ent:IsNPC() || ent:IsPlayer() then ent:TakeDamage( 5, self:GetOwner() ) end
	...
	function ENT:OnRemove()
		...
		for _, ent in ipairs( ents.FindInSphere( self:GetPos(), 512 ) ) do
			...
			local physobj = ent:GetPhysicsObject()
			if IsValid( physobj ) && !physobj:IsGravityEnabled() && !ent.IsRiftInducer then
	...
	function ENT:Draw()
		...
		if self.NewScale && self.NewScale != self.Scale then
	...
	usermessage.Hook( "RiftScale", function( um )

		local ent = um:ReadEntity()
		if !IsValid( ent ) || ent.IsDead then return end

		ent.NewScale = um:ReadFloat()

	end	)

	usermessage.Hook( "RiftDie", function( um )
	...
	local function RiftParticlesThink( particle )
		...
		if !IsValid( particle.Owner ) || !particle then
			return false
		end
