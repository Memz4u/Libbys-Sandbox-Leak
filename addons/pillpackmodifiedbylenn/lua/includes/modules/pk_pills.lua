-- "addons\\pillpackmodifiedbylenn\\lua\\includes\\modules\\pk_pills.lua"
...
if SERVER then
	...
	--Force people in build mode into pvp mode
	if ply:GetNWBool("BuildMode") then
	    ply:SetNWBool("BuildMode", false)
	    ply:ChatPrint("You've been put into PVP mode")
	end
	    ...
	    --Libby's specific modification
	    if ply:IsUserGroup("Probation") then
		ply:ChatPrint("You can't use pills on Probation")
		success = false
	    end
