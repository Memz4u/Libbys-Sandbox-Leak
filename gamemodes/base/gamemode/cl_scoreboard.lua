	...
	Think = function( self )
		...
		if ( self.PName == nil || self.PName != self.Player:Nick() ) then
			self.PName = self.Player:Nick()
                        --self.UserGroupYeah = self.Player:GetUserGroup()
                        --tcolor = team.GetColor( self.Player:Team()  )         


local ply = self.Player

                        -- I use Nick() to get username they currently have in gmod, but if you want someones actual steam name, just use Name()
                        -- This line below me sets the usernames nickname for the default scoreboard            (lenn)
			self.Name:SetText("" .. ply:Nick() .. "      " .. team.GetName( ply:Team() ) .. "  hours: " .. math.floor( ply:GetUTimeTotalTime() / 3600 ) .. "")
			-- This line below me sets the usernames nicknames color for teamcolors, for ULX / ULIB and more      (lenn)
	...
	Paint = function( self, w, h )
		...
		if ( self.Player:Team() == TEAM_CONNECTING ) then
			draw.RoundedBox( 4, 0, 0, w, h, Color( 200, 200, 200, 190 ) )
			return
		end

		if ( !self.Player:Alive() ) then
			draw.RoundedBox( 4, 0, 0, w, h, Color( 218, 218, 218, 190 ) )
			return
		end

		if ( self.Player:IsAdmin() ) then
			draw.RoundedBox( 4, 0, 0, w, h, Color( "" .. math.random(170,255) .. "", "" .. math.random(170,255) .. "",  "" .. math.random(170,255) .. "" ) )
			return
		end
